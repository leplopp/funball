...
public void mousePressed(MouseEvent e) {
	int mX  = (int) (e.getLocationOnScreen().x + camera.getX());
	int mY = (int) (e.getLocationOnScreen().y + camera.getY());
	...
	for (int i = 0; i<handler.object.size(); i++)	{
		...
		if (tempObj.getId() == ID.player)	{
			if ((int)Math.sqrt(Math.pow(mX- (int)(tempObj.getX()), 2) + Math.pow(mY - (int)(tempObj.getY()), 2 )) != 0)
